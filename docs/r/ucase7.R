# Required wealth for constant nominal vs. constant real yearly spending
# pmt       : yearly spending stated in today's CHF (real base)
...
ucase7 <- function(pmt, nper, rate, inflation) {
  ...
  # A) Constant nominal spending: pmt, pmt, ...
  ...
  # Sum of nominal spending for constant real plan (no discounting)
  sum_nominal <- if (inflation == 0) pmt * nper else pmt * ((1 + inflation)^nper - 1) / inflation
  
  # PV for constant real spending (growing annuity, growth = inflation)
  if (abs(rate - inflation) < 1e-12) {
    # limit r == g: PV = pmt * nper / (1+r)
  ...
  list(
    ...
    results = list(
      ...
      t = t,
      expense_nominal_path = expense_nominal_path,           # orange
      expense_real_nominal_path = expense_real_nominal_path  # grey (indexed)
    )
  )
}
